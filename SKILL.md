...
3. Shared state files (`internal/tasks.jsonl`, `internal/runs.jsonl`, `internal/recovery.json`, `internal/validation.json`) have one writer: the supervisor or a real runner; use the CLI where a mutation command exists. Queue mutation must refresh the queue pointer/watermark/nonterminal summary in `recovery.json` under the same queue lock; `tasks.jsonl` remains the task-state owner and recovery stores only the compact discoverability projection. The local queue lock may wait briefly for ordinary CLI contention, but it is not a Worksite-wide primary-writer lease and must still fail after a bounded timeout.
...
7. Runtime-specific carriers are adapters, not the protocol.
   - After each real delegated worker returns, persist only the provenance the runtime actually exposed (for example dispatch shape, result status, model, duration, API/tool-call count, exit reason, and hashes of the accepted handoff/artifacts) in the worker record or a compact `provenance.json`. If the carrier does not expose a child session/run id, record it as unavailable/null and never invent one. Logical task independence, observed separate worker results, and durable session identity are different claims; phrase them separately.
   - Worker-authored wall-clock timestamps are evidence only when the worker obtained them from a real clock/tool. If a worker self-reports impossible or future-dated times, retain the raw value, append a chronology reconciliation, and use observed file mtimes plus supervisor/runner events as the wall-clock authority. Do not silently rewrite history or fabricate precise replacement times.

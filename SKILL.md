...
Human-facing `output/` prose must default to the user's explicitly requested output language; if none is specified, use the current interaction language. Treat this as a hard output invariant and hidden product default, not a status banner: do not add `language_rule`, `interaction_language`, or `output_language` markers to `mission.md` or `output/` files merely to announce the language. Record language only when it is a real task constraint, such as user-requested non-default language, bilingual deliverables, translation work, or cross-language handoff risk. Keep filenames, JSON keys, commands, API names, code identifiers, external proper nouns, and stable template markers in English when that improves portability; write the explanatory body of reports, indexes, error reports, traceability notes, and next-step files in the chosen human language unless the user asks otherwise. Before writing any human-facing output, determine the chosen human language from the current request; if using helper scripts, Python string templates, or copied English templates, localize the prose before writing or rewrite it before final delivery. A primary human-facing deliverable in the wrong language is a workflow error and must be repaired, recorded in `output/90-error-report.md`, and caught during validation.
...
- When the user corrects research scope mid-run, reuse the active workspace unless the mission truly changed. Inspect partial artifacts, salvage valid material, mark superseded or excluded evidence as deprecated/background, update mission/queue/audit/next-step files, and validate against scope leakage before synthesis.
...
1. Structure validation: required files exist, JSONL parses, task ids/statuses/dependencies are valid, task output paths stay under the layout-specific worker directory (`internal/agents/<task-id>/` for v2), per-task files exist, output files are numbered and indexed, required audit files exist, and tasks/registry/status do not drift. Treat task ids as filesystem contracts: when the queue id is `T001`, the full-LLL worker directory should be exactly `internal/agents/T001/`; put semantic labels in `task.md`, registry descriptions, or artifact filenames rather than directory suffixes. When `scripts/lll.py` is available, run it against the final/resumed workdir before delivery; treat helper validation as baseline structure validation unless it explicitly implements the stricter checklist in `references/minimal-runner.md`. The helper supports `validate --mode auto|full|lite`: `auto` is conservative and treats any workdir with a layout-specific task queue as full validation; use explicit `--mode lite` for honest Lite workdirs that keep a compact checklist queue. Full mode keeps strict worker-tree checks, and lite mode accepts simple honest Lite workspaces without fake worker directories. After using helper scripts to add tasks or dependencies, verify the generated `internal/tasks.jsonl` before launching workers. If old structural debt appears, repair it honestly with evidence-labeled recovery notes rather than fabricating missing history, then re-run validation.
...
- `references/session-lessons-2026-06-13-mode-selection.md`: concrete lesson from a supervisor-heavy multi-object research run; use when tuning mode selection, Lite honesty, or context-isolation behavior.
- `references/session-lessons-2026-06-14-validation-shape.md`: concrete lesson for full LLL validation shape; use when creating task directories from `internal/tasks.jsonl`, avoiding semantic suffix drift, and ensuring `output/00-index.md` indexes itself.
- `references/session-lessons-2026-06-14-research-scope-correction.md`: concrete lesson for interrupted deep-research runs where the user changes source scope mid-run; reuse workspace, salvage partial artifacts, demote excluded sources, and validate against scope leakage.

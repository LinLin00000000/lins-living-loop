...
class LLLCliSmokeTests(unittest.TestCase):
    ...
    def test_init_task_validate(self) -> None:
        with tempfile.TemporaryDirectory() as td:
            ...
            status_help = run("task", "set-status", "--help").stdout
            self.assertIn("in_progress", status_help)
            self.assertIn("succeeded", status_help)
            invalid_status = run("task", "set-status", str(wd), "T001", "not-a-status", check=False)
            self.assertEqual(invalid_status.returncode, 2)
            self.assertIn("invalid choice", invalid_status.stderr)
            self.assertIn("commas are not split", run("checkpoint", "--help").stdout)
            self.assertIn("commas are not split", run("audit", "append", "--help").stdout)
            self.assertIn("commas are not split", run("validation", "set", "--help").stdout)

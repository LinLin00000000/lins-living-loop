...
4. If a direct file-tool write is blocked, do not fight the guard; switch to the owning CLI/tool path and record the fallback in `internal/agents/<task-id>/log.txt` or `internal/error-report.jsonl`.

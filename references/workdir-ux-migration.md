...
- Agents, runners, and supervisors use `internal/` for queues, logs, derived registry views, recovery, validation, task state, raw inputs, worker artifacts, traceability, and error records.
...
3. `templates/workdir/`: mission plus canonical recovery/validation JSON snapshots; supervisor registry/handoff are derived rather than templated.
...
Helpers should detect old workdirs loosely enough to avoid destructive writes. On explicit continuation under LLL 0.2, migrate active machine state once and do not dual-write old/new layouts; new helper/template output must not preserve redundant old-layout scaffolding.

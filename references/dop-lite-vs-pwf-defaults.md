...
- Use a timestamped, short slug directory such as `~/lll-work/YYYYMMDD-HHMMSS_short-task-name/`: `-` keeps the date/time timestamp as one field, `_` separates timestamp from slug, and slug words use `-`; avoid `YYYYMMDD_HHMMSS_slug` for new workdirs because it visually creates three fields.
...
- Minimal LLL-lite can be: root `mission.md`, `internal/tasks.jsonl`, `internal/recovery-state.md`, `internal/handoff.md`, and numbered `output/` files.
- Meaningful LLL-lite should still create `output/00-index.md`, `output/90-error-report.md`, `output/91-traceability.md`, and `output/99-next-steps.md`.

...
1. Recover from files, not chat: read `mission.md`, layout-specific queue/registry/recovery files, root task-specific deliverables, relevant worker handoffs/artifacts, and the validator task file.
...
7. Confirm that current next steps, if useful, are inside the primary deliverable or relevant deliverable.

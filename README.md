...
<lll-workdir>/
  ...
  internal/
    ...
    recovery.json            # 当前恢复快照
    validation.json          # 当前验证结论与证据指针
    inputs/
    logs/
    agents/<task-id>/
      task.md
...
新工作区不再创建 `output/`、`00-index.md`、单独的 `99-next-steps.md`、`agent-registry.md` 或 supervisor `handoff.md`。任务与 worker status 已能重建 registry；`recovery.json` 已承担当前交接。格式只遵循四条：单体当前快照用 JSON，逐行记录集合/历史用 JSONL，人类/长语义用 Markdown 或 HTML，结构化修改交给 `lll` CLI 原子执行。
...
| Validate | validator 写证据，supervisor 用 `lll validation set` 记录 `validation.json` |
| Hand off | 更新 `internal/recovery.json`；自由文本 handoff 只留在具体 worker 内 |
...
`lll status --json --compact` 是 Agent 的紧凑恢复入口：一次返回 task records/counts、`recovery.json` 与 `validation.json`，但不另存第三份状态。
...
`scripts/lll.py` 现在保留为兼容 shim，会转发到 `src/lll_cli`；旧命令 `add-task` / `set-status` 仍可用，但新文档优先使用 `task add` / `task set-status`。老名字 `scripts/dop.py` 还保留为兼容入口。旧工作区仍可发现和读取；若明确继续并使用 LLL 0.2，则一次性迁移到 JSON machine state，不双写旧 Markdown 状态。

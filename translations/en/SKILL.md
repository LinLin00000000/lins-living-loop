...
Use LLL when:
- the user explicitly says LLL, Lin's Living Loop, DOP, deep research, heavy-duty task, long-running task, deep orchestration, resumable, background agent, durable worker, or similar;
...
Current phase / new finding. Next: <next action or blocker>.
```
...
```text
Done. LLL work has passed / partially passed validation.
